...
function Cart() {
    ...
    const placeOrder = async () => {
        ...
        try {
            const response = await axios.post(`/api/v1/order-placed`, { order: cartBooks }, { headers });
    ...
    const removeCartBook = async (bookid) => {
        try {
            if (confirm("Are You sure to remove book from cart?")) {
                const response = await axios.put(`/api/v1/remove-from-cart/${bookid}`, {}, { headers });
                toast.success(response.data.message);
            }
        } catch (error) {
            toast.error(error.response.data.message)
                ...
                {cartBooks.map((items, i) => (
                    <div key={i} className='  w-full bg-zinc-800 mt-5 flex items-center px-5 py-2'>
                        <div className='md:flex justify-between items-center w-full grid grid-cols-3 gap-3 md:gap-8   '>
                            <Link to={`/view-book-details/${items._id}`} className=' order-1 md:order-none '>
                                <img src={items.url} alt='book' className=' h-20 min-w-14 ' />
                            </Link>
                            <Link to={`/view-book-details/${items._id}`} className='col-span-3 order-2 md:order-none '>
                            ...
                            <div className=' text-2xl text-zinc-100 font-semibold order-1 md:order-none '>&#8377;{items.price}</div>
